<template>
    <div class="modify_information">
        <el-dialog title="就业信息" :visible="showMask" width="30%" :close-on-click-modal="close_modal"
                   :showClose="false">
            <el-form :model="employInfo" label-width="120px">
                <el-form-item label="学号">
                    <el-input v-model="employInfo.number" disabled style="width: 80%">{{employInfo.number}}</el-input>
                </el-form-item>
                <el-form-item label="姓名">
                    <el-input v-model="employInfo.studentName" disabled style="width: 80%">{{employInfo.studentName}}</el-input>
                </el-form-item>
                <el-form-item label="专业">
                    <el-input v-model="employInfo.major" disabled style="width: 80%">{{employInfo.major}}</el-input>
                </el-form-item>
                <el-form-item label="辅导员姓名">
                <el-input v-model="employInfo.teacherName" disabled style="width: 80%">{{employInfo.teacherName}}</el-input>
                </el-form-item>
                <el-form-item label="班级">
                    <el-input v-model="employInfo.classes" disabled style="width: 80%">{{employInfo.classes}}</el-input>
                </el-form-item>
                ...
                <el-form-item label="填写时间">
                    <el-input v-model="employInfo.fillInTime"  disabled style="width: 80%">{{employInfo.fillInTime}}</el-input>
                </el-form-item>
            </el-form>
    ...
    export default {
        ...
        methods: {
            save(){
                ...
                let address = this.employInfo.province+this.employInfo.city+this.employInfo.area+this.employInfo.dtaAddress
                this.employInfo.dtaAddress = address

                this.$axios.post("/employInfo/addEmp/"+this.user.account,
                    this.employInfo,
                    {headers: {'content-type': 'application/json;charset=UTF-8'}}
                ).then(res => {
                    if (res.data.code === 200){
                        this.$message.success("就业信息提交成功")
                    }else {
            ...
            getStu(){
                this.$axios.get("/student/findOneStuByNumber/"+this.user.account).then(res => {
                    if (res.data.code === 200){
                        this.employInfo.studentName = res.data.data.name;
                        this.employInfo.major = res.data.data.major;
                        this.employInfo.classes = res.data.data.className
                        this.employInfo.studentId = res.data.data.id
                        this.getTea(res.data.data.teacherId)
                    }else {
                        this.$message.error(res.data.msg)
                    }
                })
            ...
            getTea(id){
                this.$axios.get("/teacher/findTeaById/"+id).then(res => {
                    if (res.data.code === 200){
                        this.employInfo.teacherName = res.data.data.name
                    }else {
                        this.$message.error(res.data.msg)
                    }
