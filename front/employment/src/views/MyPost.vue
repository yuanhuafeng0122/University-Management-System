<template>
    <div>
        ...
        <el-table
                :data="recruits.slice((currentPage-1)*pageSize,currentPage*pageSize)"
                border
        ...
        methods:{
            ...
            handleDownload() {
                require.ensure([], () => {
                    ...
                    export_json_to_excel(tHeader, data, '招聘信息表excel')
            ...
            getMyRec(pubId){
                this.$axios.get("/recruit/findRecByPubId/"+pubId).then(res => {
                    if (res.data.code === 200){
                        this.recruits = res.data.data
                    }else {
                        this.$message.error(res.data.msg)
